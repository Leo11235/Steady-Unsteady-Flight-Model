from .steady_calculations import calculate_air_density, calculate_gravity
...
def simulate_rocket_ascent(rocket_inputs, rocket_parameters, constants_dict):
    # ensure these dicts are clear from any previous simulations
    burn_dict = {}
    ascent_dict = {}
    flight_dict = {}
    
    helper_dict = { # time & constants
        "drag constant" : 0.5 * rocket_inputs["drag coefficient"] * pi * (rocket_inputs["rocket external radius"]) ** 2,
        "cos angle" : cos(radians(rocket_inputs["launch angle"])),
        "timestep length": rocket_parameters["burntime"] / constants_dict["number of timesteps"], 
...
def calculate_burn_kinematics(rocket_inputs, rocket_parameters, constants_dict, helper_dict):
    ...
    burn_dict = { # create flight dict, set initial parameters (time, thrust, and mass remain the same or change predictably and we can set them from the getgo)
        "time": [helper_dict["timestep length"] * i for i in range(constants_dict["number of timesteps"]+1)], # used to keep a record of exactly when everything happens
        "thrust": [helper_dict["vertical thrust"] for i in range(constants_dict["number of timesteps"]+1)], # thrust remains constant throughout burn
        "drag force": [0],
        "grav force": [init_grav_force],
        "net force": [init_net_force],
        "mass": [rocket_parameters["wet mass"] - i * rocket_parameters["total propellant mass flow rate"] * dt for i in range(constants_dict["number of timesteps"]+1)],
    ...
    for i in range(1, constants_dict["number of timesteps"]+1):
        # set variables
        ...
        drag = helper_dict["drag constant"] * calculate_air_density(constants_dict, h) * abs(v) * (-1) * v # no need to multiply by cos(angle) since v is already the vertical value
